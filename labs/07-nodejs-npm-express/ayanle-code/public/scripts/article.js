...
Article.prototype.insertRecord = function(callback) {
  $.post('/articles', {author: this.author, authorUrl: this.authorUrl, body: this.body, category: this.category, publishedOn: this.publishedOn, title: this.title})
    .then(data => {
      ...
      // 2. convert it to a JS object and assign it to a variable
      let oldRawData = JSON.parse(localStorage.getItem('rawData'));

      // 3. push response data into oldRawData object
      oldRawData.push(data);

      // 4. stringify the updated oldRawData object
      let updatedRawData = JSON.stringify(oldRawData)
